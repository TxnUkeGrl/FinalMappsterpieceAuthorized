...
export class MyGalleryService {
  ...
  updateGalleryItem(entryId: number, galleryItem: MyGallery) {
    return this.http.put<MyGallery[]>(`${this.apiUri}/updatelike/${entryId}`, {
      "objectId": galleryItem.objectId, "department": galleryItem.department, "title": galleryItem.title,
  ...
  updateVisitedStatus(entryId: number) {
    return this.http.put(`${this.apiUri}/VisitedObject/${entryId}`, {});
  }
}
